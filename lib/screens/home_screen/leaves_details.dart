...
//     // final url = Uri.parse('http://10.3.0.70:9042/api/HR/GetLeavesData');
//     print('Leaves URL $url');
//     final queryParams = {
...
//                                       leave.startDate,
//                                       leave
//                                           .endDate), // Title with formatted dates
...
//                                   _buildDetailRow(
//                                       'Holiday Type', leave.holidayType),
//                                 ],
//                               ),
//                             ),
//                           );
...
class _LeavesDetailsState extends State<LeavesDetails> {
  ...
  Future<void> fetchDataLeaves() async {
    ...
    final uri = Uri.parse(url.toString()).replace(queryParameters: queryParams);
    print('Leave URL with parameters: $uri');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final leavesDataResponse = LeavesData.fromJson(jsonResponse);

        setState(() {
          leavesData = leavesDataResponse.data;

          // Filter and print JSON data where startDate == endDate
          final filteredData = leavesData
              .where((detail) => detail.startDate == detail.endDate)
              .toList();
          filteredData.forEach((detail) {
            print(
                'Filtered Data where startDate == endDate: ${jsonEncode(detail.toJson())}');
          });
        });
      } else {
        print('Request failed with status: ${response.statusCode}');
...
// LeavesData leavesDataFromJson(Map<String, dynamic> json) =>
//     LeavesData.fromJson(json);
...
//     return Datum(
//       orgId: json["OrgId"] ?? 0, // Provide default values for null
//       empNo: json["EmpNo"] ?? '',
//       name: json["Name"] ?? '',
//       department: json["Department"] ?? '',
//       startDate: json["StartDate"] != null
//           ? DateTime.parse(json["StartDate"])
//           : DateTime.now(), // Handle null dates
