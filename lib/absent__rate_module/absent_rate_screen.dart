...
//   int get totalEmployees => totalEmp;
// }

// // ═══════════════════════════════════════════════════════════════
// // STATIC DATA
// // ═══════════════════════════════════════════════════════════════
// Future<List<AR>> fetchData(DateTime from, DateTime to, String mode) async {
...
//   @override
//   State<AbsentRateDashboard> createState() =>
//       _AbsentRateDashboardState();
// }

// class _AbsentRateDashboardState extends State<AbsentRateDashboard> {
//   DateTime _from = DateTime.utc(2026, 5, 1);
//   DateTime _to = DateTime.utc(2026, 5, 20);
...
//   //   final d = await fetchData(_from, _to, _viewMode);
//   //   setState(() {
//   //     _data = d;
//   //     _loading = false;
//   //   });
...
// }
//   List<AR> get _shown =>
//       _hideHols ? _data.where((r) => !r.isHoliday).toList() : _data;
...
//   @override
//   Widget build(BuildContext context) {
//     final logoSize = rs.tiny ? 32.0 : 40.0;
//     final titleSize = rs.tiny ? 11.0 : 14.0;
//     final subSize = rs.tiny ? 9.0 : 11.0;
...
//   final ValueChanged<bool> onHideHolsChanged;
...
//     required this.hideHols,
//     required this.onFromTap,
//     required this.onToTap,
//     required this.onModeChanged,
//     required this.onHideHolsChanged,
//     required this.onRangeSelected,
//   });

//   static const _ranges = [
//     ('All', 2026, 5, 1, 2026, 5, 20),
//     ('Wk 1', 2026, 5, 1, 2026, 5, 9),
//     ('Wk 2', 2026, 5, 11, 2026, 5, 16),
//     ('Wk 3', 2026, 5, 18, 2026, 5, 20),
//     ('Last 7d', 2026, 5, 14, 2026, 5, 20),
//   ];
...
//           // Controls — always column on tiny, row on wide
//           if (rs.wide)
//             Row(children: [
//               Expanded(
//                   child: _DateBtn(
...
//               const SizedBox(width: 10),
//               // _HolToggle(
//               //     value: hideHols, onChanged: onHideHolsChanged, tiny: rs.tiny),
//             ])
//           else
//             Column(children: [
//               Row(children: [
...
//                 Expanded(
//                     child: _DateBtn(
//                         label: 'To', date: to, onTap: onToTap, tiny: rs.tiny)),
//               ]),
//               SizedBox(height: rs.gap),
//               Row(children: [
//                 Expanded(
...
//               final f = DateTime.utc(r.$2, r.$3, r.$4);
//               final t = DateTime.utc(r.$5, r.$6, r.$7);
//               return _Chip(
//                 label: r.$1,
//                 active: from == f && to == t,
//                 onTap: () => onRangeSelected(f, t),
//                 tiny: rs.tiny,
//               );
//             }).toList(),
//           ),
...
// // class _HolToggle extends StatelessWidget {
// //   final bool value;
...
// //       child: Container(
// //         padding: EdgeInsets.symmetric(
// //             horizontal: tiny ? 7 : 10, vertical: tiny ? 6 : 8),
// //         decoration: BoxDecoration(
// //           color: value ? T.bg : T.amber.withOpacity(0.08),
// //           borderRadius: BorderRadius.circular(10),
// //           border:
// //               Border.all(color: value ? T.border : T.amber.withOpacity(0.5)),
// //         ),
// //         child: Column(
// //           crossAxisAlignment: CrossAxisAlignment.start,
// //           children: [
...
// //                   size: tiny ? 12 : 14,
// //                   color: value ? T.muted : T.amber),
// //               SizedBox(width: tiny ? 3 : 4),
// //               Text(value ? 'Hidden' : 'Shown',
// //                   style: TextStyle(
// //                       fontSize: tiny ? 9 : 11,
// //                       fontWeight: FontWeight.w700,
// //                       color: value ? T.muted : T.amber)),
...
//   final RS rs;
//   final double avgTotal, avgMale, avgFemale, sumAbsent;
//   final AR? peak, best;
//   final int wdCount;
...
//           label: 'Avg Absence',
//           value: '${avgTotal.toStringAsFixed(2)}%',
//           sub: '$wdCount working days',
//           g: [const Color(0xFFEF4444), const Color(0xFFFF7A7A)],
//           badge: avgTotal > 13 ? 'HIGH' : 'NORMAL',
//           badgeColor: avgTotal > 13 ? T.rose : T.teal),
//       _KD(
...
//     // tiny: 2x2 grid / small+mid: 2x2 grid / wide: single row
//     if (rs.wide) {
//       return Row(
//         children: cards
//             .asMap()
...
//         SizedBox(width: rs.gap),
//         Expanded(child: _KPICard(data: cards[3], rs: rs)),
//       ]),
//     ]);
//   }
// }
...
//             Container(
//               padding: EdgeInsets.symmetric(
//                   horizontal: rs.tiny ? 5 : 7, vertical: 3),
//               decoration: BoxDecoration(
...
//     final rs = widget.rs;
//     if (rs.tiny) return ['Date', 'Total %', 'Status'];
//     if (rs.wide) {
//       return [
//         'Date',
//         'Day',
...
//       ];
//     }
//     return ['Date', 'Day', 'Absent', 'Male %', 'Female %', 'Total %'];
//   }
...
//                     border: sel
//                         ? const Border(
//                             left: BorderSide(color: T.blue, width: 3))
//                         : null,
//                   ),
//                   child: _TRow(
//                     cells: _rowCells(r),
...
//                   style:
//                       TextStyle(color: T.muted, fontSize: rs.fontSize(12)))));
//     }

//     final sumAbsent = wd.fold(0.0, (s, r) => s + r.totalAbsent);
//     final sumPresent =
//         wd.fold(0.0, (s, r) => s + r.totalEmployees - r.totalAbsent);
...
//     final peak = wd.reduce((a, b) => a.totalPct > b.totalPct ? a : b);
//     final best = wd.reduce((a, b) => a.totalPct < b.totalPct ? a : b);
...
//                     color: T.ink)),
//             const Spacer(),
//             _Pill(label: '${wd.length} Days', color: T.blue, tiny: rs.tiny),
//           ]),
...
//               pct: presPct,
//               color: T.teal,
//               leftText: '${(presPct * 100).toStringAsFixed(1)}% Present',
//               rightText: '${(absPct * 100).toStringAsFixed(1)}% Absent',
//               tiny: rs.tiny),
//           SizedBox(height: rs.tiny ? 14 : 20),
//           if (rs.wide)
//             Row(children: [
//               _OStat('Total Absent', sumAbsent.toStringAsFixed(0), T.rose, rs),
//               _OStat(
//                   'Total Present', sumPresent.toStringAsFixed(0), T.teal, rs),
//               _OStat('Avg Daily', avgAbs.toStringAsFixed(0), T.amber, rs),
//               _OStat(
//                   'Peak Day',
//                   '${peak.totalPct.toStringAsFixed(1)}%\n${fmtShort(peak.date)}',
//                   T.rose,
//                   rs),
//               _OStat(
//                   'Best Day',
//                   '${best.totalPct.toStringAsFixed(1)}%\n${fmtShort(best.date)}',
//                   T.teal,
//                   rs),
//             ])
//           else
//             Wrap(
//               children: [
//                 _OStat('Absent', sumAbsent.toStringAsFixed(0), T.rose, rs),
//                 _OStat('Present', sumPresent.toStringAsFixed(0), T.teal, rs),
//                 _OStat('Avg/Day', avgAbs.toStringAsFixed(0), T.amber, rs),
//                 _OStat(
//                     'Peak', '${peak.totalPct.toStringAsFixed(1)}%', T.rose, rs),
//                 _OStat(
//                     'Best', '${best.totalPct.toStringAsFixed(1)}%', T.teal, rs),
//               ],
//             ),
//         ],
//       ),
...
//   final RS rs;
//   final double avgMale, avgFemale, avgTotal, sumMale, sumFemale;
//   const _GenderCard(
//       {required this.rs,
//       required this.avgMale,
//       required this.avgFemale,
...
//           ]),
//           SizedBox(height: rs.tiny ? 14 : 20),
//           _GBar('Male Avg Absence', avgMale, T.maleC,
//               sumMale.toStringAsFixed(0), rs),
//           SizedBox(height: rs.tiny ? 10 : 14),
//           _GBar('Female Avg Absence', avgFemale, T.femaleC,
//               sumFemale.toStringAsFixed(0), rs),
//           SizedBox(height: rs.tiny ? 10 : 14),
//           _GBar('Overall Average', avgTotal, T.totalC,
//               (sumMale + sumFemale).toStringAsFixed(0), rs),
//           SizedBox(height: rs.tiny ? 10 : 16),
//           const Divider(color: T.border),
//           SizedBox(height: rs.tiny ? 8 : 12),
//           _Insight(
//             icon: avgMale > avgFemale
//                 ? Icons.man_2_rounded
//                 : Icons.woman_2_rounded,
//             color: avgMale > avgFemale ? T.maleC : T.femaleC,
//             text:
//                 '${avgMale > avgFemale ? "Male" : "Female"} absence higher by '
//                 '${(avgMale - avgFemale).abs().toStringAsFixed(2)}% avg',
//             tiny: rs.tiny,
//           ),
//         ],
//       ),
...
//                             borderRadius: BorderRadius.circular(6))),
//                     FractionallySizedBox(
//                       widthFactor: (r.totalPct / 22).clamp(0.0, 1.0),
//                       child: Container(
//                           height: barH,
//                           decoration: BoxDecoration(
//                             gradient: LinearGradient(
//                                 colors: [sc.withOpacity(0.8), sc]),
//                             borderRadius: BorderRadius.circular(6),
//                           )),
//                     ),
//                     Positioned.fill(
...
//                           rs.tiny
//                               ? '${r.totalPct.toStringAsFixed(1)}%'
//                               : '${r.totalPct.toStringAsFixed(2)}%  (${r.totalAbsent.toStringAsFixed(0)} absent)',
//                           style: TextStyle(
...
//         Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
//           Text(leftText,
//               style: TextStyle(fontSize: tiny ? 9 : 10, color: color)),
//           Text(rightText,
...
class RS {
  ...
  double get hPad => tiny ? 8 : wide ? 28 : 14;
  double get vPad => tiny ? 10 : 18;
  double get gap => tiny ? 8 : 12;
  double get cardP => tiny ? 12 : 18;

  double fontSize(double base) =>
      tiny ? base * 0.82 : small ? base * 0.9 : base;
}
...
class _AbsentRateDashboardState extends State<AbsentRateDashboard> {
  ...
  AR? _peak() {
    final w = _wd;
    return w.isEmpty ? null : w.reduce((a, b) => a.totalPct > b.totalPct ? a : b);
  }
  AR? _best() {
    final w = _wd;
    return w.isEmpty ? null : w.reduce((a, b) => a.totalPct < b.totalPct ? a : b);
  ...
  Widget build(BuildContext context) {
    ...
    final ranges = [
      ('This Month', DateTime.utc(now.year, now.month, 1), DateTime.utc(now.year, now.month + 1, 0)),
      ('Last Month', DateTime.utc(now.year, now.month - 1, 1), DateTime.utc(now.year, now.month, 0)),
                            ...
                            if (_data.isEmpty)
                              ...
                              if (_viewMode == 'DAYWISE') ...[
                                ...
                                _GenderCard(
                                  ...
                                  avgFemaleRate: _avg((r) => r.femaleAbsentRate),
                                  avgTotalRate: _avg((r) => r.totalAbsentRate),
                                  sumMale: _sum((r) => r.maleAbsent),
                                  sumFemale: _sum((r) => r.femaleAbsent),
                                ),
...
class _ErrorView extends StatelessWidget {
  ...
  final RS rs;
  const _ErrorView({required this.message, required this.onRetry, required this.rs});
    ...
    return _Card(
      ...
      child: Column(
        ...
        children: [
          ...
          if (rs.wide)
            Row(children: [
              Expanded(child: _DateBtn(label: FieldNames.fromDate, date: from, onTap: onFromTap, tiny: rs.tiny)),
              const SizedBox(width: 10),
              Expanded(child: _DateBtn(label: FieldNames.toDate, date: to, onTap: onToTap, tiny: rs.tiny)),
              const SizedBox(width: 10),
              Expanded(child: _ModeDropdown(value: viewMode, onChanged: onModeChanged, tiny: rs.tiny)),
            ])
          else
            Column(children: [
              Row(children: [
              ...
              SizedBox(height: rs.gap),
              _ModeDropdown(value: viewMode, onChanged: onModeChanged, tiny: rs.tiny),
            ]),
          SizedBox(height: rs.tiny ? 10 : 14),
          Wrap(
            spacing: rs.tiny ? 6 : 8,
...
class _DateBtn extends StatelessWidget {
  ...
  final VoidCallback onTap;
  final bool tiny;
  const _DateBtn({required this.label, required this.date, required this.onTap, required this.tiny});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        padding: EdgeInsets.symmetric(horizontal: tiny ? 8 : 12, vertical: tiny ? 7 : 10),
        decoration: BoxDecoration(
          color: T.bg,
          borderRadius: BorderRadius.circular(10),
          border: Border.all(color: T.border),
        ...
        child: Row(children: [
          Icon(Icons.calendar_today_rounded, size: tiny ? 12 : 14, color: T.blue),
          SizedBox(width: tiny ? 5 : 8),
          Expanded(
...
class _ModeDropdown extends StatelessWidget {
  ...
  final ValueChanged<String?> onChanged;
  final bool tiny;
  const _ModeDropdown({required this.value, required this.onChanged, required this.tiny});

  @override
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: tiny ? 8 : 12, vertical: tiny ? 6 : 8),
      decoration: BoxDecoration(
        color: T.bg,
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: T.border),
          ...
          child: DropdownButton<String>(
            ...
            items: [
              DropdownMenuItem(
                  value: 'DAYWISE',
                  child: Row(children: [
                    Icon(Icons.view_day_rounded, size: tiny ? 12 : 14, color: T.blue),
                    const SizedBox(width: 4),
                    const Text('DAY WISE'),
                  ])),
              DropdownMenuItem(
                  value: 'OVERALL',
                  child: Row(children: [
                    Icon(Icons.donut_large_rounded, size: tiny ? 12 : 14, color: T.blue),
                    const SizedBox(width: 4),
                    const Text('OVERALL'),
                  ])),
            ],
...
class _Chip extends StatelessWidget {
  ...
  final bool active, tiny;
  final VoidCallback onTap;
  const _Chip({required this.label, required this.active, required this.onTap, required this.tiny});
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 200),
        padding: EdgeInsets.symmetric(horizontal: tiny ? 10 : 14, vertical: tiny ? 4 : 6),
        decoration: BoxDecoration(
          color: active ? T.blue : T.bg,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: active ? T.blue : T.border),
  ...
  Widget build(BuildContext context) {
    final cards = [
      ...
      _KD(
        ...
        label: 'Peak Day',
        value: peak == null ? 'N/A' : '${peak!.totalPct.toStringAsFixed(2)}%',
        sub: peak == null ? '-' : '${fmtShort(peak!.date)} (${dayName(peak!.date)})',
        g: [const Color(0xFFFFB545), const Color(0xFFFFD080)],
        badge: '↑ WORST',
        badgeColor: T.amber,
      ),
    ...
    if (rs.wide) {
      return Row(
        children: cards.asMap().entries.map((e) => Expanded(
          child: Padding(
            padding: EdgeInsets.only(right: e.key < cards.length - 1 ? rs.gap : 0),
            child: _KPICard(data: e.value, rs: rs),
          ),
        )).toList(),
      );
...
class _KPICard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final valueFontSize = rs.tiny ? 16.0 : rs.small ? 18.0 : 22.0;
    return Container(
      padding: EdgeInsets.all(rs.tiny ? 10 : 16),
      decoration: BoxDecoration(
        color: T.card,
        ...
        border: Border.all(color: data.g[0].withOpacity(0.15)),
        boxShadow: [
          BoxShadow(color: data.g[0].withOpacity(0.08), blurRadius: 16, offset: const Offset(0, 4))
        ],
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
            ...
            child: Icon(data.icon, color: Colors.white, size: rs.tiny ? 14 : 18),
          ),
          const Spacer(),
          Container(
            padding: EdgeInsets.symmetric(horizontal: rs.tiny ? 5 : 7, vertical: 3),
            decoration: BoxDecoration(
              color: data.badgeColor.withOpacity(0.1),
              borderRadius: BorderRadius.circular(8),
            ),
    ...
    return _Card(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        data.isEmpty
            ? SizedBox(
                height: 120,
                child: Center(child: Text('No data', style: TextStyle(color: T.muted, fontSize: rs.fontSize(12)))))
            : SizedBox(
                height: rs.tiny ? 160 : rs.small ? 190 : 230,
                child: CustomPaint(
                  painter: _ChartPainter(data: data, tiny: rs.tiny),
                  size: Size.infinite,
                ),
...
class _LegendItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 4),
      Text(label, style: const TextStyle(fontSize: 10, color: T.muted, fontWeight: FontWeight.w600)),
    ]);
  }
}
...
class _ChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gp = Paint()..color = T.border..strokeWidth = 0.8;
    final ls = TextStyle(fontSize: tiny ? 8.0 : 9.0, color: T.muted);
    ...
    double ty(double pct) {
      if (pct >= 95) return area.top;
      return area.bottom - ((pct.clamp(yMin, yMax) - yMin) / (yMax - yMin)) * area.height;
    }
    ...
    void drawFill(List<double> vals, Color c) {
      ...
      int? fi, li;
      for (var i = 0; i < data.length; i++) {
        if (data[i].isHoliday) { started = false; continue; }
        final x = tx(i);
        final y = ty(vals[i]);
        if (!started) { path.moveTo(x, y); fi = i; started = true; } else { path.lineTo(x, y); }
        li = i;
      }
      if (fi != null && li != null) {
        path.lineTo(tx(li), area.bottom);
      ...
      canvas.drawPath(path, Paint()
        ..shader = LinearGradient(
            ...
            colors: [c.withOpacity(0.18), c.withOpacity(0.01)]).createShader(area)
        ..style = PaintingStyle.fill);
    }
    ...
    void drawLine(List<double> vals, Color c) {
      ...
      for (var i = 0; i < data.length; i++) {
        if (data[i].isHoliday) { started = false; continue; }
        final x = tx(i); final y = ty(vals[i]);
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      final x = tx(i);
      if (r.isHoliday) {
        canvas.drawLine(Offset(x, area.top), Offset(x, area.bottom),
            Paint()..color = T.holiday.withOpacity(0.4)..strokeWidth = 1.2..style = PaintingStyle.stroke);
        if (!tiny) _paintText(canvas, '🏖', ls, Offset(x - 6, area.top - 14));
      } else {
        for (final (pct, c) in [
          (r.totalPct, T.totalC), (r.malePct, T.maleC), (r.femalePct, T.femaleC)
        ]) {
          final y = ty(pct);
          dotP.color = Colors.white;
          canvas.drawCircle(Offset(x, y), dotOuter, dotP);
      ...
      if (i % step == 0 || i == data.length - 1) {
        _paintText(canvas, fmtShort(r.date),
            TextStyle(fontSize: tiny ? 8.0 : 9.0, color: T.muted),
            Offset(x - 14, area.bottom + 5), width: 28, align: TextAlign.center);
      }
    }

    final ap = Paint()..color = T.border..strokeWidth = 1.5;
    canvas.drawLine(Offset(area.left, area.top), Offset(area.left, area.bottom), ap);
    canvas.drawLine(Offset(area.left, area.bottom), Offset(area.right, area.bottom), ap);
  }
  ...
  List<String> _rowCells(AR r) {
    ...
    if (rs.tiny) {
      return [
        fmtShort(r.date),
        '${r.totalPct.toStringAsFixed(1)}%',
        r.isHoliday ? 'HOL' : r.totalAbsentRate > 14 ? 'HIGH' : 'OK',
      ];
    }
    if (rs.wide) {
      return [
        ...
        r.isHoliday ? 'HOLIDAY' : r.totalAbsentRate > 14 ? 'HIGH' : 'NORMAL',
      ];
    }
    return [
      fmtShort(r.date),
    ...
    return _Card(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text('Daily Breakdown',
              style: TextStyle(
                  fontSize: rs.fontSize(14), fontWeight: FontWeight.w800, color: T.ink)),
          const Spacer(),
          _Pill(label: '${d.length} Records', color: T.blue, tiny: rs.tiny),
        ]),
        SizedBox(height: rs.tiny ? 10 : 14),
        _TRow(cells: _headers, isHeader: true, isLast: false, isHoliday: false, statusColor: null, tiny: rs.tiny),
        const Divider(height: 1, color: T.border),
        ...d.asMap().entries.map((e) {
          final i = e.key;
          final r = e.value;
          ...
          return Column(children: [
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: r.isHoliday
                      ...
                      : sel
                          ? T.blue.withOpacity(0.05)
                          : i.isEven ? const Color(0xFFF8FAFC) : Colors.transparent,
                  border: sel ? const Border(left: BorderSide(color: T.blue, width: 3)) : null,
                ),
                child: _TRow(
                  cells: _rowCells(r),
                  isHeader: false,
            ...
            const Divider(height: 1, color: T.border, indent: 10, endIndent: 10),
          ]);
        }),
        SizedBox(height: rs.tiny ? 6 : 8),
        if (!rs.tiny)
      ...
      child: Row(
        children: cells.asMap().entries.map((e) {
          ...
          return Expanded(
            ...
            child: isStatusCol && !isHeader
                ? Container(
                    padding: EdgeInsets.symmetric(horizontal: tiny ? 4 : 6, vertical: 2),
                    decoration: BoxDecoration(
                      color: textColor.withOpacity(0.1),
                      borderRadius: BorderRadius.circular(6),
                    ),
...
class _OverallCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (wd.isEmpty) {
      return _Card(
        padding: rs.cardP,
        child: Center(child: Text('No working day data',
    ...
    final absPct = 1 - presPct;
    final avgAbs = sumAbsent / wd.length;
    final peak = wd.reduce((a, b) => a.totalAbsentRate > b.totalAbsentRate ? a : b);
    final best = wd.reduce((a, b) => a.totalAbsentRate < b.totalAbsentRate ? a : b);
    ...
    return _Card(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _ProgressBar(
          ...
          pct: presPct,
          color: T.teal,
          leftText: '${(presPct * 100).toStringAsFixed(1)}% ${FieldNames.presentToday}',
          rightText: '${(absPct * 100).toStringAsFixed(1)}% ${FieldNames.totalAbsentDays}',
          tiny: rs.tiny,
        ),
        SizedBox(height: rs.tiny ? 14 : 20),
        if (rs.wide)
          Row(children: [
            _OStat(FieldNames.totalAbsentDays, sumAbsent.toStringAsFixed(0), T.rose, rs),
            _OStat(FieldNames.presentToday, sumPresent.toStringAsFixed(0), T.teal, rs),
            _OStat('Avg Daily', avgAbs.toStringAsFixed(0), T.amber, rs),
            _OStat('Peak', '${peak.totalAbsentRate.toStringAsFixed(1)}%\n${fmtShort(peak.date)}', T.rose, rs),
            _OStat('Best', '${best.totalAbsentRate.toStringAsFixed(1)}%\n${fmtShort(best.date)}', T.teal, rs),
          ])
        else
          Wrap(children: [
            _OStat(FieldNames.totalAbsentDays.split(' ').first, sumAbsent.toStringAsFixed(0), T.rose, rs),
            _OStat(FieldNames.presentToday.split(' ').first, sumPresent.toStringAsFixed(0), T.teal, rs),
            _OStat('Avg/Day', avgAbs.toStringAsFixed(0), T.amber, rs),
            _OStat('Peak', '${peak.totalAbsentRate.toStringAsFixed(1)}%', T.rose, rs),
            _OStat('Best', '${best.totalAbsentRate.toStringAsFixed(1)}%', T.teal, rs),
          ]),
      ]),
    );
  }
  ...
  Widget build(BuildContext context) {
    return _Card(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(
          'Absenteeism Rate (% of own gender)',
          style: TextStyle(fontSize: rs.fontSize(11), color: T.muted, fontWeight: FontWeight.w600),
        ),
        SizedBox(height: rs.tiny ? 8 : 10),
        _GBar(FieldNames.maleAbsentRate, avgMaleRate, T.maleC, sumMale.toStringAsFixed(0), rs),
        SizedBox(height: rs.tiny ? 8 : 12),
        _GBar(FieldNames.femaleAbsentRate, avgFemaleRate, T.femaleC, sumFemale.toStringAsFixed(0), rs),
        SizedBox(height: rs.tiny ? 8 : 12),
        _GBar(FieldNames.overallAbsentRate, avgTotalRate, T.totalC, (sumMale + sumFemale).toStringAsFixed(0), rs),
        ...
        SizedBox(height: rs.tiny ? 8 : 12),
        _Insight(
          icon: avgMaleRate > avgFemaleRate ? Icons.man_2_rounded : Icons.woman_2_rounded,
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Flexible(
            child: Text(label,
                style: TextStyle(
                    fontSize: rs.fontSize(12), fontWeight: FontWeight.w700, color: T.ink),
                overflow: TextOverflow.ellipsis)),
        Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('${pct.toStringAsFixed(2)}%',
              style: TextStyle(
                  fontSize: rs.fontSize(13), fontWeight: FontWeight.w900, color: color)),
        ]),
      ]),
      const SizedBox(height: 8),
      Stack(children: [
...
class _Insight extends StatelessWidget {
  ...
  final String text;
  final bool tiny;
  const _Insight({required this.icon, required this.color, required this.text, required this.tiny});

  @override
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: tiny ? 8 : 12, vertical: tiny ? 8 : 10),
      decoration: BoxDecoration(
        color: color.withOpacity(0.06),
        borderRadius: BorderRadius.circular(10),
        border: Border.all(color: color.withOpacity(0.2)),
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(text,
            style: TextStyle(
                fontSize: tiny ? 10 : 12, color: color, fontWeight: FontWeight.w600))),
      ]),
    );
  }
}
            ...
            child: Row(children: [
              SizedBox(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(fmtShort(r.date),
                      style: TextStyle(
                          fontSize: rs.tiny ? 9 : 11, fontWeight: FontWeight.w700, color: T.ink)),
                  Text(rs.tiny ? dayName(r.date).substring(0, 2) : dayName(r.date),
                      style: const TextStyle(fontSize: 9, color: T.muted)),
                ]),
              ),
              SizedBox(width: rs.tiny ? 6 : 10),
...
class _Pill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.symmetric(horizontal: tiny ? 7 : 10, vertical: tiny ? 3 : 4),
      decoration: BoxDecoration(
        color: color.withOpacity(0.12),
        borderRadius: BorderRadius.circular(20),
      ),
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text(rightText, style: TextStyle(fontSize: tiny ? 9 : 10, color: T.rose)),
      ]),
    ]);
  }
}